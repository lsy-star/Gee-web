...
func TestParsePattern(t *testing.T) {
	ok := reflect.DeepEqual(parsePattern2Parts("/p/:name"), []string{"p", ":name"})
	ok = ok && reflect.DeepEqual(parsePattern2Parts("/p/*"), []string{"p", "*"})
	ok = ok && reflect.DeepEqual(parsePattern2Parts("/p/*name/*"), []string{"p", "*name"})
	if !ok {
